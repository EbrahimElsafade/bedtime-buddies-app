...
export const CoursePrice = ({
  ...
  const discountTag = hasDiscount ? (
    <span className="rounded-full bg-accent/20 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-accent">
    ...
    return (
      <div className={cn('grid gap-1', className)}>
        ...
        <span className="flex flex-wrap items-center justify-center gap-2">
          ...
          {hasDiscount && (
            <span className="text-sm text-primary-foreground/60 line-through">{originalPrice}</span>
          )}
          {discountTag}
        </span>
      </div>
  ...
  return (
    <span className={cn('inline-flex flex-wrap items-center gap-1.5', className)}>
      ...
      {hasDiscount && (
        <span className="text-xs leading-none line-through opacity-70">{originalPrice}</span>
      )}
      {discountTag}
    </span>
  )
